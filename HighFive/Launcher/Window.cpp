...
namespace CEF
{
	...
	void Window::Init()
	{
		if (initialized)
			return;
		initialized = true;
		HMODULE libcef = LoadLibrary((CGlobals::Get().highFivePath + "/cef/libcef.dll").c_str());
		...
		{
			CefString(&settings.log_file).FromString((CGlobals::Get().highFivePath + "/cef/cefdebug.log").c_str());
			...
			CefString(&settings.browser_subprocess_path).FromString((CGlobals::Get().highFivePath + "/cef/csp.exe").c_str());
	...
	{
		std::string filename((CGlobals::Get().highFivePath + "/webpage.bmp").c_str());
