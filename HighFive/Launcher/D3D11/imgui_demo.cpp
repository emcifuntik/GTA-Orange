...
void ImGui::ShowTestWindow(bool* p_open)
{
    ...
    if (ImGui::BeginMenuBar())
    {
        ...
        if (ImGui::BeginMenu("Examples"))
        {
            ...
            ImGui::MenuItem("log_", NULL, &show_app_log);
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a window or a block. Tree nodes can be automatically expanded. You can also call ImGui::LogText() to output directly to the log_ without a visual output.");
            ImGui::LogButtons();
            ImGui::TreePop();
        }
    }
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
...

        {
            ...
            ImGui::SliderFloat("slider log_ float", &f2, -10.0f, 10.0f, "%.4f", 3.0f);
...
struct ExampleAppConsole
{
    ...
    void    Draw(const char* title, bool* p_open)
    {
        ...
        // Display every line as a separate entry so we can change their color or add custom widgets. If you only want raw text you can use ImGui::TextUnformatted(log_.begin(), log_.end());
...
static void ShowExampleAppLog(bool* p_open)
{
    ...
    {
        const char* random_words[] = { "system", "info", "warning", "error", "fatal", "notice", "log_" };
        log_.AddLog("[%s] Hello, time is %.1f, rand() %d\n", random_words[rand() % IM_ARRAYSIZE(random_words)], time, (int)rand());
        last_time = time;
    }

    log_.Draw("Example: log_", p_open);
}
...
static void ShowExampleAppLongText(bool* p_open)
{
    ...
    ImGui::Text("Buffer contents: %d lines, %d bytes", lines, log_.size());
    if (ImGui::Button("Clear")) { log_.clear(); lines = 0; }
    ImGui::SameLine();
    if (ImGui::Button("Add 1000 lines"))
    {
        for (int i = 0; i < 1000; i++)
            log_.append("%i The quick brown fox jumps over the lazy dog\n", lines+i);
        lines += 1000;
    }
    ImGui::BeginChild("log_");
    switch (test_type)
    {
    case 0:
        // Single call to TextUnformatted() with a big buffer
        ImGui::TextUnformatted(log_.begin(), log_.end());
        break;
    case 1:
        {
