...
public:
	...
	bool InitializeRenderClass(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, float DefaultFontSize = 14.0f, const char *DefaultFont = "Arial", UINT DefaultFontFlags = 0x0);
	void RenderText(int x, int y, int r, int g, int b, int a, bool outline, const char * szText, ...);
	void RenderText(int x, int y, int r, int g, int b, int a, bool outline, float Size, UINT Flags, const char * szText, ...);
	...
	void DrawHealthBox(int x, int y, DWORD m_dColorOut, DWORD m_dColorIn, int height, int width, float value);
