...
CConfig::CConfig()
{
	try {
		...
		gtaPath = std::string(root->FirstChildElement("gtapath")->GetText());
...
void CConfig::Save()
{
	...
	tinyxml2::XMLElement * pGtaPath = doc.NewElement("gtapath");
	pGtaPath->SetText(gtaPath.c_str());
	pRoot->InsertEndChild(pGtaPath);
