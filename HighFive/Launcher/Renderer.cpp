...
bool Dx11Renderer::InitializeRenderClass(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, float DefaultFontSize, const char *DefaultFont, UINT DefaultFontFlags)
{
...
void Dx11Renderer::RenderText(int x, int y, int r, int g, int b, int a, bool outline, const char *szText, ...)
{
...
void Dx11Renderer::RenderText(int x, int y, int r, int g, int b, int a, bool outline, float Size, UINT Flags, const char *szText, ...)
{
	if (this->bIsInitialized == false)
		return;
...
void Dx11Renderer::DrawHealthBox(int x, int y, DWORD m_dColorOut, DWORD m_dColorIn, int height, int width, float value)
{
	FillARGB(x, y, width, height, m_dColorOut);
	FillARGB(x, y, int(value * width), height, m_dColorIn);
	DrawBorder(x - 1, y - 1, (int)width + 2, height + 2, 1, D3DCOLOR_ARGB(255, 30, 30, 30));
}
